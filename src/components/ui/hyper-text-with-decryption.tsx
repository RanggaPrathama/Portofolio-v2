  ...
  return (
    ...
    >
      <AnimatePresence>
        {isHovered && (
          <motion.span
            className="absolute -inset-2 rounded-lg bg-primary/20 backdrop-blur-sm z-[-1]"
            initial={{ opacity: 0, scale: 0.8 }}
            animate={{ opacity: 1, scale: 1 }}
            exit={{ opacity: 0, scale: 0.8 }}
            style={{
              boxShadow:
                "0px 10px 25px -5px rgba(37, 99, 235, 0.3), 0px 8px 10px -6px rgba(0, 0, 0, 0.2)",
            }}
          />
        )}
      </AnimatePresence>
        ...
        {isHovered && (
          <>
            <motion.span
              ...
              animate={{ scale: 1 }}
              exit={{ scale: 0 }}
              className="absolute -top-1 -right-1 w-1.5 h-1.5 bg-blue-500 rounded-full z-20"
            />
            <motion.span
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
              exit={{ scale: 0 }}
              className="absolute -bottom-1 -left-1 w-1.5 h-1.5 bg-cyan-400 rounded-full z-20"
            />
          </>
        )}
      </AnimatePresence>
